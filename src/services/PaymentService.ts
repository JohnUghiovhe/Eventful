  ...
  static async initializePayment(data: InitializePaymentRequest): Promise<any> {
    try {
      ...
      const paystack = await this.paystackCall('post', '/transaction/initialize', {
        ...
        metadata: { eventId: data.eventId, ticketId: data.ticketId, payerId: data.payerId },
      });
  ...
  static async verifyPayment(reference: string): Promise<any> {
    try {
      ...
      const payment = await Payment.findOneAndUpdate(
        ...
        { status: 'completed', paidAt: new Date(), paystackReference: paystack.data.reference },
        { new: true }
      );

      if (payment) {
        await Analytics.findOneAndUpdate({ eventId: payment.eventId }, {
